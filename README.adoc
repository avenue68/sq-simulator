...
link:https://github.com/avenue68/sq-simulator/releases[こちら]から最新バージョンの `sq-simulator_\{version}.GTX` をダウンロードしてください。試験用盤制御用のサンプルPLCプログラムもダウンロード可能です。
...
. 実行する GX Works プロジェクトファイルを GX Works2 または GX Works3 で開きます。

. 開いた GX Works プロジェクト上で GX Simulator を起動し、PLCプログラムを GX Simulator に書き込みます。

** GX Works2：キーボードを kbd:[Alt] → kbd:[B] → kbd:[S] の順に押下すると、GX Simulator2 が起動できます。

** GX Works3：キーボードを kbd:[Alt] → kbd:[B] → kbd:[S] → kbd:[S] の順に押下すると、GX Simulator3 が起動できます。

. `sq-simulator_\{version}.GTX` ファイルを GT Designer3 (GOT2000) で開きます。
...
** キーボードの kbd:[Ctrl] + kbd:[F10] を押下すると、GT Simulator3 が起動できます。
...
実行するGX Worksプロジェクトの対象PLCが、入出力アドレスが8進の機種（MELSEC-Fシリーズ/MELSEC iQ-Fシリーズなど）か、16進の機種（MELSEC-Qシリーズ/MELSEC iQ-Rシリーズなど）かを選択します。
...
コンベヤ上にパレットが存在しないとき、各LSのON/OFFを手動で操作することができます。これは試験中、PLCプログラムの動作確認時間を短縮するために手指で直接LSをON/OFFする操作をシミュレーションするための機能です。

また、パレットが左端に到達したとき、LS2 - LS5はパレットのビスを同時には検出せず、微妙なタイムラグが生じるようになっています。これは「LS2がビスを検出したタイミングでLS3-5も検出しているとは限らない」という実機で生じ得る性質をシミュレーションするためです。
